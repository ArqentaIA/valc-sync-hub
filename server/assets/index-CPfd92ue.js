...
function readWorkbook(data) {
  return readSync(data, { type: "array" });
}
function validateColumns(rows) {
  if (!rows.length) return { ok: false, missing: [...REQUIRED_COLUMNS] };
  const keys2 = new Set(Object.keys(rows[0]).map(normalizeKey));
  const missing = REQUIRED_COLUMNS.filter((c) => !keys2.has(normalizeKey(c)));
  return { ok: missing.length === 0, missing };
}
function groupRows(rows) {
  const map = /* @__PURE__ */ new Map();
  const orderSets = /* @__PURE__ */ new Map();
  for (const row of rows) {
    ...
    const orderKey = keyMap[normalizeKey("NUMERO DE ORDEN DE TRANSFERENCIA")] ?? keyMap[normalizeKey("NÚMERO DE ORDEN DE TRANSFERENCIA")] ?? keyMap[normalizeKey("NO DE ORDEN DE TRANSFERENCIA")] ?? keyMap[normalizeKey("N° DE ORDEN DE TRANSFERENCIA")] ?? keyMap[normalizeKey("ORDEN DE TRANSFERENCIA")];
    const orderValue = orderKey ? String(row[orderKey] ?? "").trim() : "";
    if (!orderSets.has(name)) orderSets.set(name, /* @__PURE__ */ new Set());
...
function buildOutput(originalSheet, grouped) {
  const wb = utils.book_new();
  utils.book_append_sheet(wb, originalSheet, "DATOS_ORIGINALES");
  const groupedSheet = utils.json_to_sheet(grouped);
  utils.book_append_sheet(wb, groupedSheet, "RESUMEN_AGRUPADO");
...
const STEP_LABELS = ["Archivo cargado", "Validando estructura", "Procesando información", "AGRUPANDO REGISTROS", "Archivo listo"];
function Index() {
  const [steps, setSteps] = reactExports.useState(Array(5).fill("pending"));
  const [message, setMessage] = reactExports.useState("Carga tu archivo para iniciar el procesamiento automático");
  const [isProcessing, setIsProcessing] = reactExports.useState(false);
  const [hasError, setHasError] = reactExports.useState(false);
  const [resultBlob, setResultBlob] = reactExports.useState(null);
  const [resultName, setResultName] = reactExports.useState("");
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      setStep(3, "done");
      setStep(4, "done");
      setMessage(`Proceso completado correctamente · ${result.totalRows} registros · ${result.groupedRows} grupos`);
      ...
      setSteps((prev) => prev.map((s, i) => {
        ...
        if (i === (activeIdx >= 0 ? activeIdx : prev.findIndex((x) => x !== "done"))) return "error";
        return "pending";
      }));
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "relative min-h-screen w-full bg-cover bg-center bg-no-repeat", style: {
    backgroundImage: `url(${bgImage})`,
    backgroundAttachment: "fixed"
  }, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pointer-events-none absolute inset-0", style: {
      background: "linear-gradient(180deg, oklch(1 0 0 / 0.55) 0%, oklch(1 0 0 / 0.45) 50%, oklch(0.95 0.02 240 / 0.5) 100%)",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pointer-events-none absolute inset-0", style: {
      background: "radial-gradient(ellipse at 85% 30%, oklch(0.96 0.13 80 / 0.45) 0%, oklch(0.92 0.1 75 / 0.18) 35%, transparent 65%)"
    }, "aria-hidden": true }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pointer-events-none absolute inset-0", style: {
      background: "radial-gradient(circle at 50% 45%, oklch(1 0 0 / 0.18) 0%, transparent 55%)"
    }, "aria-hidden": true }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pointer-events-none absolute inset-0", style: {
      background: "linear-gradient(180deg, oklch(0.85 0.08 230 / 0.1) 0%, transparent 60%)"
    }, "aria-hidden": true }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative z-10 flex min-h-screen items-center justify-center px-4 py-6 sm:px-6 lg:px-10", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute left-6 top-6 z-20 flex items-center sm:left-10 sm:top-8", children: /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: logoValc, alt: "VALC", className: "h-12 w-auto object-contain sm:h-16 md:h-20", style: {
        filter: "brightness(1.08) contrast(1.05) drop-shadow(0 0 12px oklch(1 0 0 / 0.45)) drop-shadow(0 0 28px oklch(1 0 0 / 0.25))"
      } }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full max-w-5xl", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "glass-card relative overflow-hidden rounded-3xl p-6 sm:p-8", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pointer-events-none absolute -top-24 -right-24 h-64 w-64 rounded-full bg-[oklch(0.62_0.2_250/0.25)] blur-3xl", "aria-hidden": true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pointer-events-none absolute -bottom-24 -left-24 h-64 w-64 rounded-full bg-[oklch(0.78_0.16_75/0.15)] blur-3xl", "aria-hidden": true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("header", { className: "relative mb-5 text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("h1", { className: "text-2xl font-bold tracking-tight text-white text-shadow-soft sm:text-3xl md:text-4xl", children: [
            "Agrupa los datos",
            " ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "bg-gradient-to-r from-white via-white/90 to-[oklch(0.85_0.1_240)] bg-clip-text text-transparent", children: "en segundos." })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative flex flex-col items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("input", { ref: fileInputRef, type: "file", accept: ".xlsx,.xls", className: "hidden", onChange }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onPick, disabled: isProcessing || resultBlob !== null, className: ["group relative h-14 w-full max-w-md overflow-hidden rounded-2xl text-base font-semibold text-white transition-all", "disabled:cursor-not-allowed disabled:opacity-50", "shadow-[0_25px_60px_-15px_oklch(0.5_0.22_265/0.75)]", !isProcessing && !resultBlob ? "btn-glow hover:scale-[1.03] active:scale-[0.99]" : ""].join(" "), style: {
              background: "var(--gradient-brand)"
            }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "absolute inset-0 bg-gradient-to-b from-white/20 to-transparent opacity-0 transition group-hover:opacity-100" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "relative flex items-center justify-center gap-2.5", children: [
                isProcessing ? /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "h-5 w-5 animate-spin" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Upload, { className: "h-5 w-5" }),
                isProcessing ? "Procesando…" : "Subir archivo Excel"
              ] }),
              isProcessing && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "shimmer absolute inset-0", "aria-hidden": true })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[10px] uppercase tracking-widest text-white/50", children: ".xlsx · .xls · Hasta 100MB" }),
            fileName && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex max-w-full items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs text-white/85 ring-1 ring-white/15 backdrop-blur", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(FileSpreadsheet, { className: "h-3.5 w-3.5 shrink-0 text-emerald-300" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "truncate", children: fileName })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("ol", { className: "relative mt-6 flex items-start justify-between gap-2", children: STEP_LABELS.map((label, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(TimelineStep, { index: i, label, status: steps[i] }, label)) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: ["mt-5 flex items-start gap-3 rounded-2xl border px-4 py-3 text-sm backdrop-blur-md transition-all", hasError ? "border-red-400/30 bg-red-500/10 text-red-100" : resultBlob ? "border-emerald-400/30 bg-emerald-500/10 text-emerald-50" : "border-white/15 bg-white/5 text-white/85"].join(" "), children: [
            hasError ? /* @__PURE__ */ jsxRuntimeExports.jsx(CircleAlert, { className: "mt-0.5 h-4 w-4 shrink-0" }) : isProcessing ? /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "mt-0.5 h-4 w-4 shrink-0 animate-spin" }) : resultBlob ? /* @__PURE__ */ jsxRuntimeExports.jsx(CircleCheck, { className: "mt-0.5 h-4 w-4 shrink-0 text-emerald-300" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { className: "mt-0.5 h-4 w-4 shrink-0 text-white/60" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "leading-relaxed", children: message })
          ] }),
          hasError && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-4 flex justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: reset, className: "group relative h-12 overflow-hidden rounded-2xl px-7 text-sm font-semibold text-white transition-all hover:scale-[1.03] active:scale-[0.98] shadow-[0_15px_40px_-10px_oklch(0.6_0.22_25/0.55)]", style: {
            background: "linear-gradient(135deg, oklch(0.65 0.22 30) 0%, oklch(0.55 0.24 18) 100%)"
          }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "absolute inset-0 bg-gradient-to-b from-white/20 to-transparent opacity-0 transition group-hover:opacity-100" }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: reset, disabled: isProcessing || !fileName && !hasError, className: "text-sm font-medium text-white/60 transition hover:text-white disabled:cursor-not-allowed disabled:opacity-30", children: "↺ Reiniciar" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onDownload, disabled: !resultBlob, className: ["group relative h-11 overflow-hidden rounded-2xl px-6 text-sm font-semibold transition-all", resultBlob ? "text-white shadow-[0_15px_40px_-10px_oklch(0.55_0.2_158/0.6)] hover:scale-[1.03] active:scale-[0.98]" : "cursor-not-allowed bg-white/5 text-white/30 ring-1 ring-white/10"].join(" "), style: resultBlob ? {
              background: "var(--gradient-success)"
            } : void 0, children: [
              resultBlob && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "absolute inset-0 bg-gradient-to-b from-white/20 to-transparent opacity-0 transition group-hover:opacity-100" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "relative flex items-center justify-center gap-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-3 text-center text-[10px] uppercase tracking-[0.3em] text-white/40", children: "Procesamiento determinístico · Sin envío de datos a la nube" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Toaster2, { position: "top-right", theme: "dark", richColors: true })
  ] });
}
function TimelineStep({
  ...
  const dotClass = status === "done" ? "border-emerald-400/60 bg-emerald-500/20 text-emerald-300 shadow-[0_0_20px_oklch(0.7_0.18_152/0.4)]" : status === "active" ? "border-[oklch(0.62_0.2_250)] bg-[oklch(0.62_0.2_250/0.25)] text-white shadow-[0_0_24px_oklch(0.62_0.2_250/0.6)]" : status === "error" ? "border-red-400/60 bg-red-500/20 text-red-300" : "border-white/20 bg-white/5 text-white/40";
  const textColor = status === "done" ? "text-white/90" : status === "active" ? "text-white font-bold text-base" : status === "error" ? "text-red-200 font-semibold" : "text-white/30";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { className: "relative flex items-center gap-4 py-2", children: [
    !isLast && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: ["absolute left-[19px] top-[calc(50%+18px)] h-[calc(100%-12px)] w-px transition-colors", status === "done" ? "bg-emerald-400/40" : "bg-white/10"].join(" "), "aria-hidden": true }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: ["relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 text-xs font-bold backdrop-blur-sm transition-all", dotClass].join(" "), children: [
      status === "active" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "absolute inset-0 animate-ping rounded-full bg-[oklch(0.62_0.2_250/0.4)]", "aria-hidden": true }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "relative", children: status === "done" ? /* @__PURE__ */ jsxRuntimeExports.jsx(CircleCheck, { className: "h-4 w-4" }) : status === "active" ? /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "h-4 w-4 animate-spin" }) : status === "error" ? /* @__PURE__ */ jsxRuntimeExports.jsx(CircleAlert, { className: "h-4 w-4" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[11px]", children: index + 1 }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `text-sm transition-colors ${textColor}`, children: label })
  ] });
}
export {
  Index as component
